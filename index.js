...
//defaults
var AUTHORIZATION_ENDPOINT = 'https://appcenter.intuit.com/connect/oauth2';
var TOKEN_ENDPOINT = 'https://oauth.platform.intuit.com/oauth2/v1/tokens/bearer';
var REVOCATION_ENDPOINT = 'https://developer.api.intuit.com/v2/oauth2/tokens/revoke';

module.exports = function(opts){
  if(opts){
    if(opts.discovery){
      //
      if(opts.discovery.authorization_endpoint){
        AUTHORIZATION_ENDPOINT = opts.discovery.authorization_endpoint;
      }
      if(opts.discovery.token_endpoint){
        TOKEN_ENDPOINT = opts.discovery.token_endpoint;
      }
      if(opts.discovery.revocation_endpoint){
        REVOCATION_ENDPOINT = opts.discovery.revocation_endpoint;
      }
    }
  }
  ...
  */
  ...
  //Direct access to the class, if needed.
  libr.QboConnector = QboConnector;

  /**
    Factory method that automatically initializes the connector.
  */
  libr.qboConnect = function(client_id, client_secret, access_token, refresh_token, realm_id, opts, tokenRefreshCallback){
    var q = new QboConnector(client_id, client_secret, access_token, refresh_token, realm_id, opts);
    q.init();
    if(tokenRefreshCallback){
      q.on("token.refreshed", tokenRefreshCallback);
    }
    return q;
  }
  ...
  */
  libr.getQuickBooksAuthorizationUrl = function(client_id, redirectUri, state){
  ...
  /**
    Issues a POST request to obtain an access token.

  */
  libr.getQuickBooksAccessToken = function(client_id, client_secret, code, redirect_uri){
    return request({
      method : 'POST',
      url : TOKEN_ENDPOINT,
      headers:{
